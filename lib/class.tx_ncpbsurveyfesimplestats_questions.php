...
***************************************************************/
require_once (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('nc_pbsurveyfesimplestats') . 'lib/class.tx_ncpbsurveyfesimplestats_item.php');
...
class tx_ncpbsurveyfesimplestats_questions {
	...
	function setQuestions() {
		...
		while($aDbRow = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($dbRes))	{
			$this->questions[$aDbRow['uid']] = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('tx_ncpbsurveyfesimplestats_item');
		...
		while($aDbRow = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($dbRes))	{
			if (intval($aDbRow['answer']) != 0) {
				if(count($this->questions[$aDbRow['uid']]->aAnswers) > 0) {
					if(method_exists($this->questions[$aDbRow['uid']]->aAnswers[$aDbRow['answer']], 'setAmount')) {
						$this->questions[$aDbRow['uid']]->aAnswers[$aDbRow['answer']]->setAmount($aDbRow['number']);
					}
				} else {
					if(method_exists($this->questions[$aDbRow['uid']]->aRows[$aDbRow['row']]->aAnswers[$aDbRow['answer']], 'setAmount')) {
						$this->questions[$aDbRow['uid']]->aRows[$aDbRow['row']]->aAnswers[$aDbRow['answer']]->setAmount($aDbRow['number']);
					}
					if(method_exists($this->questions[$aDbRow['uid']]->aRows[$aDbRow['row']], 'addToTotal')) {
			...
			if(is_object($this->questions[$aDbRow['uid']])) {
