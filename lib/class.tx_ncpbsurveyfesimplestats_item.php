...
***************************************************************/
require_once (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('nc_pbsurveyfesimplestats') . 'lib/class.tx_ncpbsurveyfesimplestats_row.php');
require_once (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('nc_pbsurveyfesimplestats') . 'lib/class.tx_ncpbsurveyfesimplestats_answer.php');
...
class tx_ncpbsurveyfesimplestats_item {
	...
	function setRows($sRows) {
		...
		foreach($aRow as $intKey => $sAnswerRow) {
			$this->aRows[$intKey+1] = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('tx_ncpbsurveyfesimplestats_row');
	...
	function setAnswers($sAnswers) {
		...
		if (in_array($this->iQuestionType, array(1, 23, 2, 3, 4, 5))) {
			foreach($aLine as $intKey => $sAnswerLine) {
				...
				$this->aAnswers[$intKey+1] = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('tx_ncpbsurveyfesimplestats_answer');
			...
			foreach($this->aRows as $intRow => $oAnswer) {
				foreach($aLine as $intKey => $sAnswerLine) {
					$sValues = explode('|',$sAnswerLine);
					$this->aRows[$intRow]->aAnswers[$intKey+1] = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('tx_ncpbsurveyfesimplestats_answer');
	...
	function setAllowAdditional($bAllowAdditional, $sAdditionalText) {
		if (in_array($this->iQuestionType, array(1,3)) && $bAllowAdditional) {
			$this->aAnswers['-1'] = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('tx_ncpbsurveyfesimplestats_answer');
